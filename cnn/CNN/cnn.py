...
def k_fold(k, X_train, y_train, num_epochs=30, learning_rate=0.0001, weight_decay=0.1, batch_size=268):
...
def train(net, train_features, train_labels, test_features, test_labels, num_epochs, learning_rate, weight_decay,
    ...
    optimizer = torch.optim.SGD(net.parameters(), lr=0.0001, momentum=0.01)
...
with torch.no_grad():
    ...
    y_pred = [1 if prob[1] >= 0.1 else 0 for prob in y_pred]
   ...
   ###ROC he PR
    ...
    plt.plot(recall_dict1[1], precision_dict1[1], label='CNN AP = {0:0.2f}'.format(average_precision1[1]))
    plt.plot([0, 1], [1, 0], 'k--')
    plt.xlabel('Recall')
    ...
    plt.rcParams.update({'font.size': 15})
    plt.figure(figsize=(8, 8))
    plt.plot(fpr1[1], tpr1[1], lw=2, label='CNN AUC = {1:0.2f}'.format(1, roc_auc1[1]))
