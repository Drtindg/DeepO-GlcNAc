...
df = pd.read_csv(r'E:/桌面整理/文档/师哥课题/real.csv')
# define input string
...
class CNN_SEAttention(nn.Module):
    def __init__(self):
        super().__init__()
        # batch*1*21*21（每次会送入batch个样本，输入通道数1（黑白图像），图像分辨率是28x28）
        # 下面的卷积层Conv2d的第一个参数指输入通道数，第二个参数指输出通道数，第三个参数指卷积核的大小
...
class CNN_SEAttention_LSTM(nn.Module):
    def __init__(self):
        ...
        self.se2 = SELayer(20)
        # LSTM
        self.lstm = nn.LSTM(input_size=20*12*12, hidden_size=512, num_layers=2)  # 输入 (1, batchsize, 20*12*12)
...
model_path = r'E:/桌面整理/文档/师哥课题/best_model.pth'
...
# micro
precision_dict["micro"], recall_dict["micro"], _ = precision_recall_curve(out_test_labels.cpu().numpy().ravel(),
                                                                          scores.ravel())
...
# plt.legend(loc="lower right")
# # plt.savefig('Two-class-ROC.jpg', bbox_inches='tight')
# plt.show()
